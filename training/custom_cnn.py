...
def train_model():
    ...
    # Train model
    history = model.fit(dataset_train, epochs=3, validation_data=dataset_valid.take(150))
    ...
    model.save(".\\models\\trained_models\\custom_cnn")
    model.save_weights(".\\models\\checkpoints\\custom_cnn\\custom_cnn")
...
def load_model():
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Conv2D(56,
                               ...
                               padding='same', activation='relu'),
        tf.keras.layers.MaxPooling2D()])
    model.load_weights("./models/checkpoints/custom_cnn/custom_cnn")
